...
<pre>
	** pageContext(PageContext) : JSP 페이지와 관련된 모든 정보를 가진 객체
	requset(HttpServletRequest) : 요청과 클라이언트에 대한 정보를 캡슐화한 객체
	response(HttpServletResponse) : 응답과 관련된 모든 정보를 캡슐화한 객체
	out(JSPWriter) : 출력버퍼에 데이터를 기록하거나 버퍼를 제어하기 위해 사용되는 출력 스트림
	session(HttpSession) : 한 세션 내에서 발생하는 모든 정보를 갭슐화한 객체
	appication(ServlertContext) : 컨텍스트(어플리케이션)와 서버에 대한 정보를 가진 객체
	<%=application.hashCode() %>
	config(ServletConfig) : 서블릿 등록과 관련된 전보를 가진 객체
