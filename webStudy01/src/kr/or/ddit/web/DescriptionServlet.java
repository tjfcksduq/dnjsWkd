...
/**
 * Servlet
 * : 자바 기반의 확장 CGI 방식을 이용한 웹 프로그래밍 기술 스팩이면서 동시에 API
 * 
 * 서블릿 개발 단계
 * 1. HttpServlet을 상속하여 구현
 * 2. 컴파일 -> 웹 어플리케이션의 classpath에 배포(/WEB_INF/classes)
 * 3. 등록
 ...
 * : 서블릿 스팩에 따라 개발된 웹 어플리케이션을 운영할 주체(관리자)
 * : 서블릿의 생명주기를 관리하는 역할
 *   (서블릿의 생명주기 내에서 특정 이벤트가 발생시 해당 콜백 메소드를 호출)
 * : 싱글턴, 요청이 쓰레드로 처리되며, 해당 쓰레드는 폴링 정책에 따라 관리됨  
 *  
 ...
 *   1) 서블릿 매필 정보를 통해 현재 요청을 처리할 서블릿을 검색
